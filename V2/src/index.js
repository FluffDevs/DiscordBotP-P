...
// verification.js attache volontairement de nombreux listeners interactionCreate
// indépendants (un par type de composant/étape du flux) ; augmenter la limite
// évite le MaxListenersExceededWarning sans indiquer une vraie fuite mémoire.
  ...
  for (const file of files) {
    try {
      ...
      if (command && command.name) {
        ...
        logger.info(`Chargée commande: ${command.name}`, { noTelegram: true });
      } else {
  ...
  for (const file of files) {
    try {
      ...
      if (command && command.data && command.data.name) {
        ...
        logger.info(`Chargée slash-commande: ${command.data.name}`, { noTelegram: true });
      } else {
