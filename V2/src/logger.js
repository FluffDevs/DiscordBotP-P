...
export function commandInvocation(details) {
  try {
    ...
    if (typeof details === 'string') {
      ...
      // build a clear, structured multi-line summary
      const time = timestamp();
      const user = details.userTag ? `${details.userTag} (${details.userId || 'unknown'})` : (details.userId || 'unknown');
      const guild = details.guildId ? `${details.guildName ?? ''} (${details.guildId})`.trim() : 'DM';
      const channel = details.channelId ? `${details.channelName ?? ''} (${details.channelId})`.trim() : 'inconnu';
      const cmd = details.commandName || details.command || 'inconnue';
