...
export default {
  ...
  const bak = path.join(DATA_DIR, 'verifications.json.bak.' + stamp);
      ...
      for (const [id, thread] of threadsMap) {
        if (id === keepThreadId) continue; // conserver ce thread (épinglé, ou le plus ancien à défaut) : ni sauvegardé ni supprimé
      ...
      const summary = 'Flush forum: trouvés=' + results.total + ' gardé=' + (keepThreadId ? keepThreadId : 'aucun') + ' sauvegardés=' + (results.saved || 0) + ' supprimés=' + results.deleted + ' erreurs=' + results.errors.length;
